...
const StatCard = ({
  ...
  <div className="min-w-32 sm:min-w-40 bg-white/75 rounded-lg shadow-[0px_8px_16px_-4px_rgba(12,26,36,0.04)] border border-white backdrop-blur-md p-3 flex flex-col gap-2">
    ...
    <p className={`text-lg sm:text-xl font-bold tracking-tight ${valueClass}`}>
      {isLoading ? (
  ...
  return (
    <div className="relative bg-white rounded-md shadow-[0px_8px_16px_0px_rgba(143,149,178,0.15)] p-4 sm:p-5">
      <div className="flex items-center justify-between">
        <h3 className="text-neutral-900 text-sm sm:text-base lg:text-lg font-bold">Scheme Distribution</h3>
      </div>
      <div className="my-3 h-px w-full bg-slate-200" />
      <div className="text-slate-900 text-xl sm:text-2xl lg:text-3xl font-bold">
        {isLoading ? (
      ...
      <div className="mt-4">
        <div className="h-32 sm:h-40 md:h-48 lg:h-56 xl:h-64 overflow-x-auto">
          <div className="min-w-[500px] sm:min-w-[600px] md:min-w-0 flex h-full items-end justify-center gap-1 sm:gap-2 md:gap-3 lg:gap-4 px-2">
            {bars.map((b, idx) => {
              const hPct = (b.v / maxV) * 100;
              return (
                <div key={idx} className="flex h-full flex-col-reverse items-center relative group">
                  <div
                    className={`w-3 sm:w-4 md:w-5 lg:w-6 xl:w-7 rounded-md ${b.color} ${isLoading ? 'animate-pulse' : ''}`}
                    style={{ height: `${hPct}%` }}
                    title={`${b.m}: ${b.v}M`}
                  />
                  <span className="mb-1 text-[9px] sm:text-[10px] md:text-xs text-slate-400">{b.m}</span>
                  {b.badge && !isLoading && (
                    <div className="absolute -top-6 sm:-top-8 opacity-0 group-hover:opacity-100 transition-opacity">
                      <div className="px-2 sm:px-3 py-1 rounded-md bg-blue-950 text-white text-[9px] sm:text-[10px] md:text-xs font-bold whitespace-nowrap">
                        {b.badge}
                      </div>
                    </div>
                  )}
      ...
      <div className="mt-4 sm:mt-5 flex flex-wrap items-center gap-2 sm:gap-3 md:gap-4">
...
function MetricCircle({
  ...
  return (
    <div className="flex items-center gap-2 sm:gap-3">
      <div
        className="relative h-10 w-10 sm:h-12 sm:w-12 rounded-full"
        style={{
      ...
      >
        <div className="absolute inset-1 rounded-full bg-white shadow-[0_8px_16px_rgba(0,0,0,0.06)] grid place-items-center">
          <div className={`h-6 w-6 sm:h-7 sm:w-7 rounded-full ${iconBg} grid place-items-center`}>
      ...
      <div className="leading-tight">
        ...
        {sub && <div className="text-[10px] sm:text-[11px] text-neutral-500">{sub}</div>}
      </div>
    </div>
  );
}
  ...
  return (
    <div className="relative bg-white rounded-2xl border border-slate-200 shadow-[0_12px_16px_-4px_rgba(12,26,36,0.04)] p-4 sm:p-5">
      <div className="mb-1 flex items-start justify-between">
        <div>
          <h3 className="text-sm sm:text-base lg:text-lg font-bold text-neutral-900">Application Status Overview</h3>
      ...
      <div className="mt-3 grid grid-cols-1 lg:grid-cols-2 gap-4 min-h-[250px] sm:min-h-[280px]">
        <div className="flex items-center justify-center">
          {isLoading ? (
            <div className="h-32 w-32 sm:h-40 sm:w-40 lg:h-56 lg:w-56 bg-gray-200 rounded-full animate-pulse"></div>
          ) : (
            <div className="relative h-32 w-32 sm:h-40 sm:w-40 lg:h-56 lg:w-56">
              ...
              <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center">
                <div className="text-xl sm:text-2xl lg:text-3xl font-bold text-slate-900">{approvedPercent}%</div>
                <div className="text-[10px] sm:text-xs text-slate-500">Approved</div>
              </div>
            </div>
          )}
        </div>
        <div className="flex flex-col items-start justify-center gap-3 sm:gap-4 lg:gap-6">
          <MetricCircle
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2 sm:gap-3 lg:gap-5 text-[10px] sm:text-xs md:text-sm text-slate-700">
        <div className="flex items-center gap-1 sm:gap-2">
          <span className="h-2 w-2 sm:h-2.5 sm:w-2.5 rounded-full bg-red-500" /> 
          <span className="whitespace-nowrap">Rejected ({statusCounts.rejected})</span>
        </div>
        <div className="flex items-center gap-1 sm:gap-2">
          <span className="h-2 w-2 sm:h-2.5 sm:w-2.5 rounded-full bg-blue-600" /> 
          <span className="whitespace-nowrap">Under Review ({statusCounts.under_review})</span>
        </div>
        <div className="flex items-center gap-1 sm:gap-2">
          <span className="h-2 w-2 sm:h-2.5 sm:w-2.5 rounded-full bg-emerald-500" /> 
          <span className="whitespace-nowrap">Approved ({statusCounts.approved})</span>
        </div>
      </div>
    </div>
  );
...
function RecentApplicationsTable({ applications, isLoading }: { 
  ...
  const getStatusBadge = (status: string) => {
    ...
    return (
      <span className={`px-2 py-1 rounded-full text-[10px] sm:text-xs font-medium ${config.bg} ${config.text} whitespace-nowrap`}>
        {config.label}
      </span>
    );
  };
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg p-4 sm:p-6">
      <h3 className="text-base sm:text-lg font-semibold text-gray-800 mb-4">Recent Applications</h3>
      {isLoading ? (
        ...
        <div className="space-y-3">
          {applications.map((app) => (
            <div key={app.id} className="flex flex-col sm:flex-row sm:justify-between sm:items-center p-3 border border-gray-200 rounded-lg hover:bg-gray-50 gap-2 sm:gap-4">
              <div className="flex-1 min-w-0">
                <div className="font-medium text-gray-900 text-sm sm:text-base truncate">{app.fullName}</div>
                <div className="text-xs sm:text-sm text-gray-500 truncate">{app.organizationName}</div>
...
function InspectorActivityTable() {
  ...
  const getStatusBadge = (status: string) => {
    return status === 'Active' ? (
      ...
      <span className="inline-flex items-center px-2 sm:px-2.5 py-0.5 rounded-full text-[10px] sm:text-xs font-medium bg-yellow-100 text-yellow-800">
        • Inactive
      </span>
    );
  };
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg p-4 sm:p-6">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4 gap-2">
        <h3 className="text-base sm:text-lg font-semibold text-gray-800">Inspectors Activity Overview</h3>
        <button className="text-xs sm:text-sm text-blue-600 hover:text-blue-700 text-left sm:text-right">
      ...
      <div className="block sm:hidden space-y-4">
        {inspectorData.map((inspector) => (
          <div key={inspector.id} className="border border-gray-200 rounded-lg p-4 hover:bg-gray-50 cursor-pointer">
            <div className="flex justify-between items-start mb-2">
              <div className="font-medium text-gray-900">{inspector.name}</div>
              {getStatusBadge(inspector.status)}
            </div>
            <div className="grid grid-cols-2 gap-2 text-xs text-gray-600">
              <div>Assigned: {inspector.items}</div>
              <div>Completed: {inspector.assigned}</div>
              <div>Pending: {inspector.pending}</div>
              <div>Last Visit: {inspector.lastVisit}</div>
            </div>
          </div>
        ))}
      </div>
        ...
        <table className="w-full">
          <thead className="bg-gray-50 rounded-lg">
            <tr>
              ...
              <th className="px-3 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Applications Assigned</th>
              <th className="px-3 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Visits Completed</th>
              <th className="px-3 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Pending</th>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {inspectorData.map((inspector, index) => (
              <tr key={inspector.id} className="hover:bg-gray-50 cursor-pointer">
                <td className="px-3 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-900">{inspector.id}</td>
                <td className="px-3 sm:px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{inspector.name}</td>
                <td className="px-3 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-900">{inspector.items}</td>
                <td className="px-3 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-900">{inspector.assigned}</td>
                <td className="px-3 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-900">{inspector.pending}</td>
                <td className="px-3 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-900">{inspector.lastVisit}</td>
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg p-4 hover:shadow-xl transition-all duration-300">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 gap-2">
        <h3 className="text-base sm:text-lg font-semibold text-gray-800">Inspector Activity Map</h3>
        <button className="text-xs sm:text-sm text-blue-600 hover:text-blue-700 flex items-center space-x-1 self-start sm:self-center">
      ...
      <div className="relative bg-gray-100 rounded-xl overflow-hidden h-64 sm:h-80 lg:h-96 xl:h-[450px]">
        <div className="w-full h-full bg-gradient-to-br from-blue-100 to-green-100 flex items-center justify-center">
          <div className="text-gray-500 text-center px-4">
            <MapPin className="w-12 h-12 sm:w-16 sm:h-16 mx-auto mb-4 text-gray-400" />
            <p className="text-sm sm:text-lg">Interactive Map</p>
            <p className="text-xs sm:text-sm">Inspector locations would be shown here</p>
          </div>
        </div>
        
        {/* Mock location pins for Lahore - Adjusted for mobile */}
        {[
          { cls: 'top-16 left-8 sm:top-32 sm:left-20', label: "Quaid-e-Azam Campus Inspector" },
          { cls: 'top-12 left-32 sm:top-24 sm:left-64', label: 'Cavalry Ground Office' },
          { cls: 'top-8 right-16 sm:top-20 sm:right-32', label: "Gulberg Inspector" },
          { cls: 'bottom-16 right-12 sm:bottom-32 sm:right-28', label: 'DHA Branch Inspector' },
          { cls: 'bottom-20 left-1/4 sm:bottom-40 sm:left-1/3', label: 'Model Town Inspector' },
        ].map((m, i) => (
          <div key={i} className={`absolute ${m.cls} group cursor-pointer z-10`}>
            <div className="relative">
              <div className="w-5 h-5 sm:w-6 sm:h-6 bg-red-500 rounded-full shadow-lg flex items-center justify-center transform hover:scale-110 transition-transform border-2 border-white">
                <div className="w-2 h-2 sm:w-3 sm:h-3 bg-white rounded-full"></div>
              </div>
              <div className="absolute -top-8 sm:-top-10 left-1/2 -translate-x-1/2 bg-gray-800 text-white px-2 py-1 rounded text-[9px] sm:text-xs whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity z-20 max-w-32 sm:max-w-none">
                {m.label}
              </div>
            </div>
          </div>
...
const SupervisorDashboardContent: React.FC = () => {
  ...
  return (
    <div className="relative min-h-screen bg-gray-50">
      {/* Header Section */}
      <div className="bg-gradient-to-r from-blue-600 to-teal-500 text-white p-4 sm:p-6 relative">
        <div className="mb-4 sm:mb-6">
          <h2 className="text-xl sm:text-2xl font-bold text-white">Dashboard</h2>
          <p className="text-white text-sm sm:text-base">Welcome to Supervisor Dashboard</p>
          {error && (
            <div className="mt-2 flex items-center gap-2 text-yellow-200">
              <AlertCircle className="h-4 w-4 flex-shrink-0" />
              <span className="text-xs sm:text-sm">Using demo data - API connection failed</span>
            </div>
          )}
        </div>
      </div>

      {/* Stats Cards Section - Mobile Responsive */}
      <div className="mb-6 sm:mb-8 absolute top-16 sm:top-20 left-4 sm:left-12 right-4 sm:right-8 z-10">
        <div className="flex gap-3 sm:gap-4 overflow-x-auto pb-4 scrollbar-hide">
          ...
          <StatCard
            ...
            value={stats?.inspectorStats?.totalInspectors?.toLocaleString() || '0'}
            icon={<Users />}
          ...
          <StatCard
            ...
            value={stats?.inspectorStats?.activeInspectors?.toLocaleString() || '0'}
            icon={<UserCheck />}
          ...
          <StatCard
            ...
            value={`${stats?.inspectorStats?.averageCompletionRate?.toFixed(1) || '0'}%`}
            icon={<TrendingUp />}
      ...
      {/* Main Content Area */}
      <div className="pt-24 sm:pt-32 px-4 sm:px-6 pb-6 sm:pb-8 space-y-6 sm:space-y-8">
        {/* Charts Section */}
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 sm:gap-8">
          {/* Recent Applications - Takes 2 columns on XL screens, full width on smaller */}
          ...
          {/* Inspector Map - Takes 1 column on XL screens, full width on smaller */}
