...
// Helper function to get purpose label from ID and category - UPDATED
const getPurposeLabel = (purposeId: string, category: string): string => {
  ...
  if (!categoryPurposes) {
    ...
    return `${category} - ${purposeId}`; // Fallback to show both category and ID
  }
  
  const purpose = categoryPurposes.find((p: { id: string; }) => p.id === purposeId);
  return purpose ? purpose.label : `${category} - ${purposeId}`; // Better fallback
};
...
const PreviewDocuments: React.FC<PreviewDocumentsProps> = ({ extractedData, documents, onBack }) => {
  ...
  // SIMPLIFIED Initialize form data with extracted values
  useEffect(() => {
    ...
    try {
      ...
      if (savedPurpose) {
        ...
        if (purposeData.purposeId && purposeData.category) {
          ...
          // If we only have category, use that
      ...
      // Try to use the raw value if JSON parsing fails
    ...
    const initialFormData = {
      ...
      purpose: purposeDisplay, // Use the resolved purpose display
    };
  ...
  }, [extractedData, userInfo]); // Removed selectedPurpose from dependencies

  // DEBUGGING: Add this temporary useEffect to log purpose changes
  useEffect(() => {
    console.log("=== PURPOSE DEBUGGING ===");
    console.log("selectedPurpose state:", selectedPurpose);
    console.log("formData.purpose:", formData.purpose);
    console.log("localStorage selectedPurpose:", localStorage.getItem("selectedPurpose"));
  ...
  const submitApplication = async () => {
    ...
    try {
      ...
      submissionFormData.append('scheme', 'scheme1');
